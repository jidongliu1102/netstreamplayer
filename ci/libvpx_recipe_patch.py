...
class VPXRecipe(Recipe):
    ...
    def build_arch(self, arch):
        with current_directory(self.get_build_dir(arch.arch)):
            ...
            # NDK r28c moved arm_neon.h to clang/lib/include (not sysroot).
            # Pass extra include so libvpx configure finds it.
            clang_inc = join(self.ctx.ndk.llvm_prebuilt_dir, 'lib', 'clang', '19', 'include')
            flags = [
                '--target=' + TARGETS[arch.arch],
                f'--extra-cflags=-I{clang_inc}',
                '--enable-pic',
                '--enable-vp8',
                '--enable-vp9',
                '--enable-static',

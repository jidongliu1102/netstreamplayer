...
class MjpegReader(threading.Thread):
    ...
    def __init__(self, url, username='', password='', on_frame=None, on_error=None, **kwargs):
        ...
        self._target_framerate = 10.0
        self._last_frame_time = 0.0
        # 高分辨率跳帧：每 N 帧才更新显示（但录像可保存所有帧）
        self._display_skip = 1  # 初始 1（每帧都显示）
    ...
    def _process_frame(self, jpeg_data):
        ...
        if frame_size > 150000:  # > ~150KB → 高分辨率帧
            self._display_skip = 2  # 每 2 帧才显示 1 帧
        elif frame_size > 80000:
        ...
        if self._display_counter % self._display_skip != 0:
            # 不显示但仍更新缓存
            with self._lock:
                self._latest_frame = jpeg_data
                self._frame_count += 1
...
class CaptureManager:
    ...
    MAX_RECORDING_FRAMES = 1500  # 最大帧缓存，防内存溢出
    ...
    def record_frame(self, jpeg_data):
        ...
        if elapsed >= self.MAX_RECORDING_DURATION:
            ...
            self._last_recording_info = info
            return 'TIMEOUT_STOPPED'
        # 帧缓存上限保护
        if len(self._recording_frames) >= self.MAX_RECORDING_FRAMES:
            return 'RECORDING'  # 丢弃多余帧，不报错
...
class MjpegDisplay(Image):
    ...
    def disconnect(self):
        ...
        if self._fps_update_clock:
            ...
            self._fps_update_clock = None
        # 清理临时 JPEG 文件
        if hasattr(self, '_temp_jpeg_name'):
            try:
                os.unlink(self._temp_jpeg_name)
            except Exception:
                pass
            del self._temp_jpeg_name
            self._temp_jpeg = None
        self.is_connected = False
        self.frame_rate = 0
        self._frame_times = []
        ...
        try:
            ...
            try:
                ...
                if core_img.texture:
                    ...
                    raise ValueError('CoreImage returned None texture')
            except Exception as e:
                # CoreImage 解码失败，改用临时文件加载
                print(f'[WARN] CoreImage 解码失败 ({e}), 尝试文件加载...')
                import tempfile
                if not hasattr(self, '_temp_jpeg'):
                    self._temp_jpeg = tempfile.NamedTemporaryFile(
                        suffix='.jpg', delete=False)
                    self._temp_jpeg_name = self._temp_jpeg.name
                with open(self._temp_jpeg_name, 'wb') as f:
                    f.write(jpeg_data)
                self.source = self._temp_jpeg_name
                self.reload()
                print(f'[INFO] 文件加载方式成功')
...
class SourceListScreen(Screen):
    ...
    def restore_config(self):
        """弹出文件列表，选择备份配置恢复。
        扫描多个常见路径，显示所有文件供选择。
        在 Android 上，若无 MANAGE_EXTERNAL_STORAGE 权限则引导用户授权。"""
        if platform == 'android':
            self._restore_config_android()
        else:
            self._restore_config_desktop()

    def _restore_config_desktop(self):
        """桌面版：扫描当前目录。"""
        all_files = []
        scan_path = os.getcwd()
        try:
            for fname in os.listdir(scan_path):
                if os.path.isfile(os.path.join(scan_path, fname)):
                    all_files.append(fname)
        except Exception:
            pass
        all_files.sort(reverse=True)
        self._show_file_list(all_files, scan_path)

    def _restore_config_android(self):
        """Android 版：检查 MANAGE_EXTERNAL_STORAGE 权限后扫描目录。"""
        # 检查 MANAGE_EXTERNAL_STORAGE 权限
        has_perm = False
        try:
            from jnius import autoclass
            Environment = autoclass('android.os.Environment')
            has_perm = Environment.isExternalStorageManager()
        except Exception:
            pass
        ...
        if not has_perm:
            ...
            box.add_widget(Label(
                text='需要「所有文件访问」权限才能读取备份文件。\n请点击下方按钮前往设置开启。',
                font_size=dp(14), halign='center'))
            btn_settings = Button(
                text='⚙ 前往设置', size_hint_y=None, height=dp(50),
                font_size=dp(16),
                background_color=(0.2, 0.6, 0.2, 1),
                background_normal='',
            )
            def _open_settings(btn):
                try:
                    from jnius import autoclass
            ...
            box.add_widget(btn_settings)
            btn_close = Button(text='取消', size_hint_y=None, height=dp(40))
            box.add_widget(btn_close)
            popup = Popup(title=_('restore_config'), content=box,
                          size_hint=(0.9, 0.5))
    ...
    def _show_file_list(self, all_files, scan_path):
        """显示文件列表 UI。"""
        box = BoxLayout(orientation='vertical', spacing=dp(5))
        info = f'扫描: {scan_path}\n找到 {len(all_files)} 个文件'
        box.add_widget(Label(text=info, size_hint_y=None, height=dp(30),
                             font_size=dp(12)))
        if not all_files:
            box.add_widget(Label(text='未找到文件', font_size=dp(16)))
        else:
            self._build_file_list_ui(box, all_files, scan_path)
        btn_close = Button(text='取消', size_hint_y=None, height=dp(40))
        box.add_widget(btn_close)
        popup = Popup(title=_('restore_config'), content=box,
                      size_hint=(0.9, 0.8))
    ...
    def _build_file_list_ui(self, box, all_files, scan_path):
        """构建文件列表（复用）。"""
        ...
        for fname in all_files:
            ...
            btn.bind(on_release=lambda btn, fn=fname, d=scan_path: (
                self._do_restore(os.path.join(d, fn))
            ))
            list_box.add_widget(btn)
        scroll.add_widget(list_box)
        box.add_widget(scroll)

    def _do_restore(self, filepath):
        """执行配置恢复。"""
        app = App.get_running_app()
        ok, msg = app.source_manager.restore_from(filepath)
        if ok:
            self.refresh_list()
